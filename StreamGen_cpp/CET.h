...
struct CET {
  ...
  void addition(const int tid, const vector<unsigned short> & itemset, const FP & FPTree);
	int addHelp(const int tid,
			   ...
			   const FP & FPTree,
			   TreeNode & node);
  void deletion(const int tid, const vector<unsigned short> & itemset, const FP & FPTree);
  ...
  void getOccurrence(const size_t previousPrefix, const map<FPNodePtr, pair<int, int> > & previousOccurrence, const FP & FPTree, map<FPNodePtr, pair<int, int> > & myOccurrence);
