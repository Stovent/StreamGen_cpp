		...
		if ( currentPrefix.size() == 0 ) { //if still at the root
				...
				pos != myOccurrence.end(); ++pos ) 
			{
				...
				while ( posFP->parent != 0 && posFP->item <= maxEnd ) { //while not the root
					map<unsigned short, unsigned short>::iterator pos2 = inverse.find( posFP->item );
					if ( pos2 != inverse.end() ) {
						node.myChildren[posFP->item].mySupport += tempCount;
						node.myChildren[posFP->item].myTidSum += tempTidSum;

						map<FPNodePtr,pair<int,int> >::iterator pos3 = occurrenceFamily[pos2->second].find(posFP);
		...
		pos1 != node.myChildren.end() && myStartPosition < myItemset.size(); ++pos1 )
	{
		...
		//fact: pos1->first == myItemset[myStartPosition]
			...
			if ( pos1->second.isInfrequent ) { //need not to pass down, redundant
				...
				for ( HashClosed::const_iterator pos = p.first; pos != p.second; ++pos ) {
					if ( pos->second.first == pos1->second.mySupport && isSubset(pos->second.second, currentPrefix ) ) {
				...
				if ( stillClosed ) {
					...
					map<FPNodePtr, pair<int,int> > childOccurrence;
					getOccurrence((isOccurrenceLoaded? myPrefix : previousPrefix), (isOccurrenceLoaded? myOccurrence : previousOccurrence),	FPTree, childOccurrence);

					pos1->second.childrenSupport = initializeHelp(pos1->second, childOccurrence, pos1->first, maxEnd, tempBool);
				...
				if ( pos1->second.isClosed ) {
					pair<HashClosed::iterator, HashClosed::iterator> p = closedItemsets.equal_range(pos1->second.myTidSum-tid); //old value

					for ( HashClosed::iterator pos = p.first; pos != p.second; ++pos ) {
						if ( pos->second.first == (pos1->second.mySupport-1) && pos->second.second == currentPrefix  ) {
					...
					closedItemsets.insert(make_pair(pos1->second.myTidSum, make_pair(pos1->second.mySupport,currentPrefix)));
					pos1->second.isClosed = true;
				}
		...
		//two types: passed down from parent, or newly frequent in this level
			...
			//check left-pruning
			pair<HashClosed::const_iterator, HashClosed::const_iterator> p = closedItemsets.equal_range(pos1->second.myTidSum);
			...
			if ( stillClosed ) {
				...
				map<FPNodePtr, pair<int,int> > childOccurrence;
				getOccurrence( myPrefix + 1, occurrenceFamily[inverse[pos1->first]], FPTree, childOccurrence);
...
void CET::cleanNode(TreeNode & node)
{
  ...
  if (node.isClosed) {
    ...
    for (HashClosed::iterator pos = p.first; pos != p.second; ++pos) {
      if (pos->second.first == (node.mySupport) && pos->second.second == currentPrefix) {
        closedItemsets.erase(pos);
        break;
      }
    }
  ...
  TreeNode & node)
{
  ...
  unsigned short myStartPosition = 0;
  for (Family::iterator pos1 = node.myChildren.begin(); pos1 != node.myChildren.end() && myStartPosition < myItemset.size(); ++pos1) {
    if (pos1->first > myItemset.back()) break; //done! (not possible)

    //case 1, an old node, not in the myItemset, need to proprogate the update down ONE LEVEL
    else if (pos1->first < myItemset[myStartPosition]) {
      if (pos1->second.isInfrequent || pos1->second.isUnpromising ||
        !existNewInFrequent)
      {
      ...
      for (int j = myStartPosition; j < myItemset.size(); j++) {
        if (myIsNew[j]) {
          ...
          //this children could not be frequent before: it is not in the deleted itemset
        }
      }
    }

    //case 2, an old node, in the itemset, still frequent after deletion need to delete items down
    //fact: pos1->first == myItemset[myStartPosition]
    else if (!myIsNew[myStartPosition]) {
      if (pos1->second.isUnpromising) {//if originally unpromising				
        ...
        for (HashClosed::const_iterator pos = p.first; pos != p.second; ++pos) {
          if (pos->second.first == pos1->second.mySupport && isSubset(pos->second.second, currentPrefix) && !isPrefix(currentPrefix, pos->second.second)) {
            //notice: avoid my descendents, i.e., my support and tidsum have been updated, but not my children
            pos1->second.isUnpromising = true;
            stillClosed = false;
            break;
          }
        ...
        if (!stillClosed) { //if becomes unpromising, prune
          cleanNode(pos1->second);
          pos1->second.myChildren.erase(pos1->second.myChildren.begin(), pos1->second.myChildren.end());
        }
        else { //else, pass the update down
          if (myStartPosition + 1 < myItemset.size()) {
            deleteHelp(tid, myItemset, myIsNew, myStartPosition + 1, FPTree, pos1->second);
          }
  ...
  if (itemsToErase.size() != 0) {
    for (set<unsigned short>::iterator pos9 = itemsToErase.begin(); pos9 != itemsToErase.end(); ++pos9) {
  ...
  if (needCount) {
    ...
    for (Family::iterator pos = node.myChildren.begin(); pos != node.myChildren.end(); ++pos) {
